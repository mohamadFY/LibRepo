...
public class PatronController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Patron> getPatronById(@PathVariable long id) throws ChangeSetPersister.NotFoundException {
    ...
    public ResponseEntity<? extends Object> deleteById(@PathVariable long id) throws Exception {
        try {
            return new ResponseEntity<Boolean>(patronService.deletePatron(id), HttpStatus.OK);
        }
        catch (Exception ex){
            return new ResponseEntity<String>("CAN'T DELETE PATRON",HttpStatus.BAD_REQUEST);
        }
    }
